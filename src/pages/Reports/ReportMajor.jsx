...
import { getReportMajor } from "~/services";
...
function ReportMajor() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getReportMajor(token, requestJWT);
  ...
  return (
    <div className={cx('wrapper')}>
      ...
      <p className={cx('title-report')}>
        Báo cáo, thống kê số sinh viên trúng tuyển theo ngành trong năm {new Date().getFullYear()}
      </p>
    </div>
  );
}
